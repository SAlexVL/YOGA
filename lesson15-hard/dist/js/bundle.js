...
function tabs() {
  ...
  var hideTabC = function hideTabC(a) {
  ...
  var showTabC = function showTabC(b) {
...
function timer() {
  var deadline = '2019-05-19';

  var getTimeR = function getTimeR(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        zero = 0;

    if ("".concat(t) > "".concat(zero)) {
      //функция для добавления 0 в одиночные цифры
      var zeroStr = function zeroStr(a) {
        if (a.toString().length <= 1) {
          a = "".concat(zero).concat(a);
        }
  ...
  var setClockF = function setClockF(id, endtime) {
    var timer = document.getElementById(id),
        hours = timer.querySelector('.hours'),
        minutes = timer.querySelector('.minutes'),
        seconds = timer.querySelector('.seconds');

    var updateClockF = function updateClockF() {
    ...
    var timeInterval = setInterval(updateClockF, 1000);
  };

  setClockF('timer', "".concat(deadline));
}
